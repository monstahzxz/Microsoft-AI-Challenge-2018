...
def convertIntoEmbed(inputFile, outputFile):
	...
	for line in inFile:
		tokens = line.strip().lower().split('\t')
		queryId, query, passage, label, passageId = tokens[0], tokens[1], tokens[2], tokens[3], tokens[4]
		...
		outFile.write(queryVec + '\t' + passageVec + '\t' + label + '\n')
...
if __name__ == '__main__':
	...
	convertIntoEmbed('data.tsv','evaluate.tsv') #evaluate.tsv contains queryId, queryVec, passageVec, label, passageId