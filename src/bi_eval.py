...
def shuffle(X):
	...
	return query_X, passage_X


def model(data_file, result_file):
	file = open(data_file, 'r', encoding='utf-8')
	out_file = open(result_file, 'w', encoding='utf-8')
	#query_X = []
	#passage_X = []
	#labels_X = []
	X = []
	...
	for example in file:
		...
		X[0].append(query_vec)
		X[0].append(passage_vec)
		#X[i].append(label)
		...
		#for j in range(int(18721 / minibatch_size) + 1):
		#print(j)
		que = qX
		pas = pX
		#lab = lX
		#lab = np.array(lab).reshape(len(lab),1)
		pred = sess.run([Z4], feed_dict = {query_X : que, passage_X : pas})
		#e_A = sess.run(A2, feed_dict = {query_X : que, passage_X : pas, labels : lab})
		#if (j + 1) % 70 == 0:
			#print('Example : ' + str(l + 1) + '.. Cost of epoch ' + str(k + 1) + ' batch ' + str(j + 1))
		#print(pred[0])
		#preds.append(1 / ( 1 + math.exp(-pred[0])))
		preds.append(1 - 1 / (1 + math.exp(pred[0])))
			#print(e_A)
			#print(lab)
			#print(e_P)
			#print(e_Q)
			#print(lab)
			#saver.save(sess, "D:/NNs/RNN/Word embeddings/work/models2/model.ckpt")
		
		#saver.save(sess, "D:/NNs/RNN/Word embeddings/work/models2/model.ckpt")			
		if i == 9:
			out_file.write(str(query_id))
			for o in range(10):
				out_file.write('\t' + str(preds[o]))
			out_file.write('\n')
			preds = []
			i = -1
...
if __name__ == '__main__':
	...
	evaluation_file = 'eval2_unlabelled_embedded.tsv'
	result_file = 'answer.tsv'
	

	X = model(evaluation_file, result_file)
	#print('\nTrained\n')
